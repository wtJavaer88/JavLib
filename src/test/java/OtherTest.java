...
public class OtherTest
{
    ...
    public void t2() throws Exception
    {
        ...
        System.out.println(PatternUtil
                .getLastPattern("http://j8vlib.com/cn/vl_star.php?s=ci&page=2",
                        "s=.*&").replace("s=", "").replace("&", ""));
        System.out.println(PatternUtil
                .getLastPattern("http://j8vlib.com/cn/vl_star.php?s=ci",
                        "s=.*&?").replace("s=", "").replace("&", ""));
        System.out.println(BasicStringUtil.removeRightString("abbb,dd,d,d,",
                ","));
        // AliasHelper.init();
        System.out.println(PatternUtil.getFirstPattern("adsf组组长郭",
                "[\u4e00-\u9fa5]"));
        System.out.println(PatternUtil.getFirstPattern("AAA-004 完全", ".*?\\s")
                .trim());
    ...
    private void checkEmptyImages()
    {
        ...
        for (File f : new File(p).listFiles())
        {
            if(f.length() == 0)
            {
                ...
                Map selectSqlMap = DbExecMgr
                        .getSelectSqlMap("SELECT * FROM JVIDEO WHERE ID='" + id
                                + "'");
                System.out.println(selectSqlMap.get("IMAGE"));
                BasicFileUtil.writeFileString("uncomplete.txt",
                        selectSqlMap.get("IMAGE") + "\r\n", null, true);
            }
        }
    }
    ...
    {
        String p = "D:\\Users\\wnc\\Programs\\sts-bundle\\projects\\javlib\\log5w.txt";
        ...
        for (String string : readFrom)
        {
            if(string.contains("未完成"))
            {
                ...
                count++;
                BasicFileUtil.writeFileString("uncomplete.txt",
                        PatternUtil.getFirstPattern(string, "http:.*?\\s")
                                .trim() + "\r\n", null, true);
            }
        }
